...
local function HarvestCoke(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingCoke[source] == true then
			...
			if Config.RawLimit ~= -1 and coke.count >= Config.RawLimit then
...
local function TransformCoke(source)
	...
	SetTimeout(Config.TimeToProcess, function()
		...
		if PlayersTransformingCoke[source] == true then
			...
			if poochQuantity > Config.PoochLimit then
			...
			elseif cokeQuantity < Config.ExchangeTake then
				...
				xPlayer.addInventoryItem('coke_pooch', Config.ExchangeGive)
	...
	SetTimeout(Config.TimeToSell, function()
		...
		if PlayersSellingCoke[source] == true then
			...
			elseif poochQuantity < 5 then
				...
				xPlayer.removeInventoryItem('coke_pooch', Config.PoochSaleQty)
				xPlayer.addAccountMoney('black_money', Config.CokeSellDealer * Config.PoochSaleQty)
				TriggerClientEvent('esx:showNotification', source, _U('sold_five_coke'))
...
local function HarvestMeth(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingMeth[source] == true then
			...
			if Config.RawLimit ~= -1 and meth.count >= Config.RawLimit then
	...
	SetTimeout(Config.TimeToProcess, function()
		...
		if PlayersTransformingMeth[source] == true then
			...
			elseif methQuantity < Config.ExchangeTake then
				...
				xPlayer.addInventoryItem('meth_pooch', Config.ExchangeGive)
	...
	SetTimeout(Config.TimeToSell, function()
		...
		if PlayersSellingMeth[source] == true then
			...
			elseif poochQuantity < 5 then
				...
				xPlayer.addAccountMoney('black_money', Config.MethSellDealer)
				TriggerClientEvent('esx:showNotification', source, _U('sold_one_meth'))
				
				SellMeth(source)
			else
				xPlayer.removeInventoryItem('meth_pooch', Config.PoochSaleQty)
				xPlayer.addAccountMoney('black_money', Config.MethSellDealer * Config.PoochSaleQty)
				TriggerClientEvent('esx:showNotification', source, _U('sold_five_meth'))
...
local function HarvestWeed(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingWeed[source] == true then
			...
			if Config.RawLimit ~= -1 and weed.count >= Config.RawLimit then
	...
	SetTimeout(Config.TimeToProcess, function()
		...
		if PlayersTransformingWeed[source] == true then
			...
			elseif weedQuantity < Config.ExchangeTake then
				...
				xPlayer.addInventoryItem('weed_pooch', Config.ExchangeGive)
	...
	SetTimeout(Config.TimeToSell, function()
		...
		if PlayersSellingWeed[source] == true then
			...
			elseif poochQuantity < 5 then
				...
				xPlayer.removeInventoryItem('weed_pooch', Config.PoochSaleQty)
				xPlayer.addAccountMoney('black_money', Config.WeedSellDealer * Config.PoochSaleQty)
				TriggerClientEvent('esx:showNotification', source, _U('sold_five_weed'))
...
local function HarvestOpium(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingOpium[source] == true then
			...
			if Config.RawLimit ~= -1 and opium.count >= Config.RawLimit then
	...
	SetTimeout(Config.TimeToProcess, function()
		...
		if PlayersTransformingOpium[source] == true then
			...
			elseif opiumQuantity < Config.ExchangeTake then
				...
				xPlayer.addInventoryItem('opium_pooch', Config.ExchangeGive)
	...
	SetTimeout(Config.TimeToSell, function()
		...
		if PlayersSellingOpium[source] == true then
			...
			elseif poochQuantity < 5 then
				...
				xPlayer.removeInventoryItem('opium_pooch', Config.PoochSaleQty)
				xPlayer.addAccountMoney('black_money', Config.OpiumSellDealer * Config.PoochSaleQty)
				TriggerClientEvent('esx:showNotification', source, _U('sold_five_opium'))
