...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = _U('field_wear'), value = 'field_wear' }
	}

	if grade == 'associate' then
		table.insert(elements, {label = _U('associate_wear'), value = 'associate_wear'})
	elseif grade == 'soldier' then
		table.insert(elements, {label = _U('soldier_wear'), value = 'soldier_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'citizen_wear' then
			...
			-- 			TriggerEvent('esx_cartel:updateBlip')
		...
		if
			data.current.value == 'associate_wear' or
			data.current.value == 'soldier_wear' or
			data.current.value == 'lieutenant_wear' or
			data.current.value == 'boss_wear' or
			data.current.value == 'cartel_wear' or
			data.current.value == 'field_wear'
		then
...
function GetAvailableVehicleSpawnPoint(station, partNum)
	local spawnPoints = Config.CartelStations[station].Vehicles[partNum].SpawnPoints
...
-- 						TriggerServerEvent('esx_cartel:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
-- 						OpenBodySearchMenu(closestPlayer)
-- 					elseif action == 'handcuff' then
-- 						TriggerServerEvent('esx_cartel:handcuff', GetPlayerServerId(closestPlayer))
-- 					elseif action == 'drag' then
-- 						TriggerServerEvent('esx_cartel:drag', GetPlayerServerId(closestPlayer))
-- 					elseif action == 'put_in_vehicle' then
-- 						TriggerServerEvent('esx_cartel:putInVehicle', GetPlayerServerId(closestPlayer))
-- 					elseif action == 'out_the_vehicle' then
-- 						TriggerServerEvent('esx_cartel:OutVehicle', GetPlayerServerId(closestPlayer))
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_cartel:getVehicleFromPlate', function(owner, found)
				if found then
...
-- 			TriggerServerEvent('esx_cartel:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
-- 	ESX.TriggerServerCallback('esx_cartel:getVehicleInfos', function(retrivedInfo)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_cartel:forceBlip')
end)
...
-- RegisterNetEvent('esx_cartel:unrestrain')
-- AddEventHandler('esx_cartel:unrestrain', function()
-- 	if IsHandcuffed then
-- 		local playerPed = PlayerPedId()
-- 		IsHandcuffed = false
...
-- AddEventHandler('esx_cartel:drag', function(copID)
...
-- AddEventHandler('esx_cartel:OutVehicle', function()
-- 	local playerPed = PlayerPedId()
...
-- 				TriggerEvent('esx_cartel:hasEnteredEntityZone', closestEntity)
-- 				LastEntity = closestEntity
-- 			end
-- 		else
-- 			if LastEntity ~= nil then
-- 				TriggerEvent('esx_cartel:hasExitedEntityZone', LastEntity)
-- 				LastEntity = nil
-- 			end
-- 		end
-- 	end
...
Citizen.CreateThread(function()
	while true do
		...
		-- if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'cartel' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'cartel_actions') then
		-- 	if Config.MaxInService == -1 then
		...
		-- 	CurrentTask.Busy = false
		-- end
		-- if IsControlJustPressed(0, Keys['F10']) and GetLastInputMethod(2) and PlayerData.job ~= nil and PlayerData.job.name == 'cartel' then
		-- 	SendcartelDistressSignal()
		-- end
	end
end)
...
-- 	end
-- end
-- function SendcartelDistressSignal()
-- 	local playerPed = GetPlayerPed(-1)
-- 	local coords	= GetEntityCoords(playerPed)

-- 	ESX.ShowNotification(_U('distress_sent'))
-- 	TriggerServerEvent('esx_phone:send', 'cartel', _U('cartel_distress_message'), false, {
-- 		x = coords.x,
-- 		y = coords.y,
-- 		z = coords.z
-- 	})
...
-- RegisterNetEvent('esx_cartel:updateBlip')
-- AddEventHandler('esx_cartel:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_cartel:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_cartel:spawned')
	end
