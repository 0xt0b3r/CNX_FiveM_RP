  ...
  MySQL.Async.fetchAll(
    ...
    function(result)
      ...
      if foundNumber then
        ...
        TriggerClientEvent('esx:showNotification', source, 'This number is invalid')
      end
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
    ...
    TriggerClientEvent('esx:showNotification', _source, 'You have transfered ~g~$' .. amount .. '~s~ to ' .. targetXPlayer.name)
